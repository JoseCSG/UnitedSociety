...
class OrganizationModel {
    ...
    func unfollowOrg(id_usuario: String, id_org: String){
        let url = "http://127.0.0.1:5000/org/unfollow/\(id_org)/\(id_usuario)"
        AF.request(url, method: .patch, headers: HTTPHeaders(headers)).responseData { res in
            print(res.response)
        }
    }
